...
const createClubLectura = async (req, res) => {
  if (req.method === 'POST') {
    ...
    try {
      ...
      res.status(201).json({ message: 'Club de lectura creado exitosamente', id: result.rows[0].idclub });
    } catch (error) {
      
      res.status(500).json({ message: 'Hubo un error al crear el club de lectura' });
    }
  } else {
