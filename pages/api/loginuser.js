...
const loginUser = async (req, res) => {
  console.log('Consultando usuario...API', req.body)
  if (req.method === 'POST') {
    ...
    try {
      ...
      const user = await db.query(
        'SELECT * FROM reader_club_users WHERE documento = $1',
        [documento]
      )
      ...
      if (user.rows.length === 0) {
        return res
          .status(401)
          .json({ message: 'Documento o contraseña incorrecta' })
      }
      ...
      const token = jwt.sign(
        { userId: idUsuario, username: nombreUsuario },
        SECRET_KEY,
        { expiresIn: '1h' }
      )
      ...
      res.setHeader(
        'Set-Cookie',
        `auth=${token}; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=${
          60 * 60
        }`
      )
